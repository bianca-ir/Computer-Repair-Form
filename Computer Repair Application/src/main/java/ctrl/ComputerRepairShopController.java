...
public class ComputerRepairShopController {
    ...
    @FXML
    private TableView<ComputerRepairRequest> repairedResults;
    ...
    @FXML
    public void repairedComputersHandler(ActionEvent actionEvent) {
            RequestStatus selected=requestStatus.getSelectionModel().getSelectedItem();
            List<ComputerRepairRequest> results=shopServices.getRequestsByStatus(selected);
            repairedResults.getItems().clear();
            repairedResults.getItems().addAll(results);
